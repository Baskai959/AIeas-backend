...
--         （auction_lot 沿用 auction_id BIGINT，与技术方案 10.2 节口径一致）
...
CREATE TABLE IF NOT EXISTS `user` (
  ...
  `phone`         VARCHAR(32)     NOT NULL                COMMENT '手机号（登录主键）',
  `nickname`      VARCHAR(64)     NOT NULL                COMMENT '昵称',
  `avatar_url`    VARCHAR(512)    DEFAULT NULL            COMMENT '头像 URL',
  `role`          VARCHAR(16)     NOT NULL DEFAULT 'user' COMMENT '角色：user/merchant/admin',
  `status`        VARCHAR(16)     NOT NULL DEFAULT 'active' COMMENT '账号状态：active/frozen/deleted',
...
INSERT INTO `user` (`id`, `phone`, `nickname`, `role`, `status`) VALUES
  (1, '13800000001', '系统管理员', 'admin',    'active'),
  (2, '13800000002', '演示商家',   'merchant', 'active'),
  (3, '13800000003', '买家小明',   'user',     'active'),
  (4, '13800000004', '买家小红',   'user',     'active');
