-- =============================================================================
-- seed_1000_buyers.sql
-- 继续上一批 seed_100_buyers.sql 的规则，额外生成 1000 个可登录买家账号。
--
-- ID 段：
--   user.id       : 91000101 ~ 91001100
--   API/domain ID : u_91000101 ~ u_91001100
--
-- 登录信息：
--   account  : loadbuyer0101 ~ loadbuyer1100
--   password : Passw0rd!
--
-- 可重放性：
...
-- 统一成 4 位补零账号格式：loadbuyer0101 ~ loadbuyer1100。
-- 同时修复旧版脚本误用 LPAD(seq.n, 3, '0') 后，1000~1100 这段序号被截断的问题。
UPDATE `user` AS u
LEFT JOIN `user` AS c
...
WHERE u.`id` BETWEEN 91000101 AND 91001100
  AND u.`role` = 'buyer'
  AND c.`id` IS NULL
  AND (
    u.`account` <> CONCAT('loadbuyer', LPAD(u.`id` - 91000000, 4, '0'))
...
SELECT
  ...
  'ACTIVE' AS `status`
FROM (
  SELECT hundreds.i * 100 + tens.i * 10 + ones.i + 101 AS n
  FROM (
...
WHERE seq.n BETWEEN 101 AND 1100
ORDER BY seq.n;
...
FROM `user`
WHERE `id` BETWEEN 91000101 AND 91001100
  AND `role` = 'buyer';
