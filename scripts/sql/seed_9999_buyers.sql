-- =============================================================================
-- seed_9999_buyers.sql
-- 生成 / 补齐买家账号到 9999 号，适合本地开发、报名/出价压测。
--
-- ID 段：
--   user.id       : 91000001 ~ 91009999
--   API/domain ID : u_91000001 ~ u_91009999
--
-- 登录信息：
--   account  : loadbuyer0001 ~ loadbuyer9999
--   password : Passw0rd!
--
-- 可重放性：
--   * 先 UPDATE 已存在的本 ID 段 buyer，统一 4 位账号 / 昵称格式
--   * 再 INSERT IGNORE 补齐缺失用户，重复执行不会改写其它业务数据
--   * 若目标 ID / account / phone 已被其它用户占用，对应行会被跳过
--
-- 注意：
--   * 本脚本只插入 / 修复 user 表，不创建 deposit_ledger
...
-- 统一成 4 位补零账号格式：loadbuyer0001 ~ loadbuyer9999。
UPDATE `user` AS u
LEFT JOIN `user` AS c
...
WHERE u.`id` BETWEEN 91000001 AND 91009999
  AND u.`role` = 'buyer'
  AND c.`id` IS NULL
  AND (
    u.`account` <> CONCAT('loadbuyer', LPAD(u.`id` - 91000000, 4, '0'))
...
SELECT
  ...
  'ACTIVE' AS `status`
FROM (
  SELECT thousands.i * 1000 + hundreds.i * 100 + tens.i * 10 + ones.i AS n
  FROM (
...
WHERE seq.n BETWEEN 1 AND 9999
ORDER BY seq.n;

SELECT
  COUNT(*) AS `buyerCount`,
  9999 - COUNT(*) AS `missingCount`,
  MIN(CONCAT('u_', `id`)) AS `firstBuyerId`,
  MAX(CONCAT('u_', `id`)) AS `lastBuyerId`
FROM `user`
WHERE `id` BETWEEN 91000001 AND 91009999
  AND `role` = 'buyer';
