...
const (
	DefaultOrderPayTimeout           = 20 * time.Minute
	DefaultOrderTimeoutScanInterval  = 30 * time.Second
	DefaultOrderTimeoutScanBatchSize = 100
)
...
func (s *OrderService) SetUserRepository(users repository.UserRepository) {
	if s == nil {
		return
	}
	s.users = users
