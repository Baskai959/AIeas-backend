...
func NewMarketplaceService(auctions repository.AuctionRepository, sessions repository.LiveSessionRepository, deposits repository.DepositRepository, orders repository.OrderRepository, users repository.UserRepository) *MarketplaceService {
	return &MarketplaceService{auctions: auctions, sessions: sessions, deposits: deposits, orders: orders, users: users}
}

func (s *MarketplaceService) SetRealtime(realtime repository.AuctionRealtimeStore) {
	if s == nil {
		return
	}
	s.realtime = realtime
...
func (s *MarketplaceService) SetOnlineCounter(hub OnlineCounter) {
	if s == nil {
		return
	}
	s.hub = hub
...
func (s *MarketplaceService) LiveSessionView(ctx context.Context, session domain.LiveSession) domain.LiveSessionView {
	...
	if s != nil && s.hub != nil && session.ActiveAuctionID != 0 {
