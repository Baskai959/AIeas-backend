...
func NewMySQLDepositRepository(db *gorm.DB, resolver ContextDBResolver) *MySQLDepositRepository {
	return &MySQLDepositRepository{db: db, resolveDB: resolver}
}
...
func depositRowFromDomain(deposit domain.DepositLedger) depositRow {
	return depositRow{
		...
		RelatedOrderID: cloneUint64Ptr(deposit.RelatedOrderID),
...
func (r depositRow) toDomain() domain.DepositLedger {
	return domain.DepositLedger{
		...
		Amount:         r.Amount,
		Status:         r.Status,
		RelatedOrderID: cloneUint64Ptr(r.RelatedOrderID),
		Remark:         r.Remark,
...
func (r *MemoryDepositRepository) Create(ctx context.Context, deposit *domain.DepositLedger) error {
	...
	deposit.RelatedOrderID = cloneUint64Ptr(deposit.RelatedOrderID)
...
var _ depositports.DepositRepository = (*MySQLDepositRepository)(nil)
var _ depositports.DepositRepository = (*MemoryDepositRepository)(nil)
