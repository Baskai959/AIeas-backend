...
type LiveSessionRealtimeStore = livesessionports.LiveSessionRealtimeStore
...
func (s *MemoryLiveSessionRealtimeStore) SetActiveAuction(ctx context.Context, sessionID uint64, auctionID uint64) error {
	_ = ctx
	if sessionID == 0 {
		return nil
	}
	...
	s.active[sessionID] = auctionID
	return nil
}

func (s *MemoryLiveSessionRealtimeStore) ClearActiveAuction(ctx context.Context, sessionID uint64) error {
	return s.SetActiveAuction(ctx, sessionID, 0)
}

func (s *MemoryLiveSessionRealtimeStore) ActiveAuction(ctx context.Context, sessionID uint64) (uint64, bool, error) {
	_ = ctx
	if sessionID == 0 {
		return 0, false, nil
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	auctionID := s.active[sessionID]
	return auctionID, auctionID != 0, nil
}
