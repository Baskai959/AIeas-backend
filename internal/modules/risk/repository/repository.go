...
func NewMySQLRiskRepository(db *gorm.DB, resolver ContextDBResolver) *MySQLRiskRepository {
	return &MySQLRiskRepository{db: db, resolveDB: resolver}
}
...
func (r *MemoryRiskRepository) SnapshotEvents() []domain.RiskEvent {
	r.mu.RLock()
	defer r.mu.RUnlock()
	out := make([]domain.RiskEvent, 0, len(r.riskEvents))
	for _, event := range r.riskEvents {
...
var _ riskports.RiskRepository = (*MySQLRiskRepository)(nil)
var _ riskports.RiskRepository = (*MemoryRiskRepository)(nil)
