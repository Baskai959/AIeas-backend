...
func newPreRejectFixture(t *testing.T, rule json.RawMessage, status domain.AuctionStatus) preRejectFixture {
	...
	hammerSvc := NewHammerServiceWithDeps(HammerServiceDeps{Auctions: auctionRepo, Orders: orderRepo, Deposits: depositRepo, Realtime: rt, Tx: repository.NoopTxManager{}})
	auctionSvc := NewAuctionServiceWithDeps(AuctionServiceDeps{Auctions: auctionRepo, Tx: repository.NoopTxManager{}, Realtime: rt, AuctionConfig: cfg})
	bidSvc := NewBidServiceWithDeps(BidServiceDeps{Bids: bidRepo, Auctions: auctionRepo, Realtime: rt, Risk: riskSvc, Hammer: hammerSvc, Config: cfg, Configs: configRepo, RiskControls: riskControlSvc})
...
func TestBidServiceLuaResultUpdatesLocalCache(t *testing.T) {
	...
	cached, ok := fx.bids.CachedBidRealtimeStateForTest(fx.auctionID, time.Now())
	if !ok {
...
func TestBidServiceSamePriceInflightGateRejectsBeyondLimit(t *testing.T) {
	...
	counter := fx.bids.SamePriceGateCounterForTest(key)
	if counter == nil {
...
func TestBidServiceInflightHighestPriceGateRejectsLowerPrice(t *testing.T) {
	...
	highCounter := fx.bids.SamePriceGateCounterForTest(highKey)
	if highCounter == nil {
...
func TestBidServiceSamePriceGateSweepsIdleZeroCounters(t *testing.T) {
	...
	if !fx.bids.SamePriceGateExistsForTest(key) {
		t.Fatalf("expected gate entry to exist before sweep")
	}
	fx.bids.SweepSamePriceGateForTest(fx.auctionID, time.Now().Add(-samePriceGateIdleTTL-time.Second), time.Now())
	if fx.bids.SamePriceGateExistsForTest(key) {
