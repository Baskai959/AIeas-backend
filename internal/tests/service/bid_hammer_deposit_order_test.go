...
func TestDepositEnrollIsIdempotent(t *testing.T) {
	...
	state, ok, err := fixture.realtime.GetAuctionState(ctx, fixture.auctionID)
	if err != nil || !ok {
		t.Fatalf("read realtime state: ok=%v err=%v", ok, err)
	}
	if state.ParticipantCount != 1 {
		t.Fatalf("expected one participant after idempotent enrollment, got %+v", state)
	}
}
...
func TestBidServiceDebouncesHighestBidLiveAgentHook(t *testing.T) {
	...
	lot, err := fixture.auctionRepo.FindByID(ctx, fixture.auctionID)
	if err != nil {
	...
	first, err := fixture.bids.Place(ctx, PlaceBidInput{RequestID: "hook-bid-1", AuctionID: fixture.auctionID, BidderID: "u_1001", UserRole: domain.RoleBuyer, Price: 1100, ExpectedCurrentPrice: expectedCurrentPrice(1000)})
	...
	hook.assertNoCall(t, 15*time.Millisecond)
	got := hook.wait(t)
...
func TestBidServiceAutoBlacklistFailureDoesNotBlockRejection(t *testing.T) {
	...
	fixture.bids.SetRiskService(failingRisk)
	setBidBlacklistStrategy(t, fixture.bids, domain.BlacklistStrategyConfig{
		Enabled:               true,
		FrequencyEnabled:      false,
		MissingDepositEnabled: true,
...
type realtimeAuctionFixture struct {
	...
	auctionRepo *repository.MemoryAuctionRepository
	realtime    *repository.MemoryRealtimeStore
}
...
func newRealtimeAuctionFixtureWithRule(t *testing.T, cfg appconfig.AuctionConfig, endOffset time.Duration, reservePrice int64, rule json.RawMessage) realtimeAuctionFixture {
	...
	hammerSvc := NewHammerServiceWithDeps(HammerServiceDeps{Auctions: auctionRepo, Orders: orderRepo, Deposits: depositRepo, Realtime: realtime, Tx: repository.NoopTxManager{}})
	auctionSvc := NewAuctionServiceWithDeps(AuctionServiceDeps{Auctions: auctionRepo, Tx: repository.NoopTxManager{}, Realtime: realtime, AuctionConfig: cfg})
	bidSvc := NewBidServiceWithDeps(BidServiceDeps{Bids: bidRepo, Auctions: auctionRepo, Realtime: realtime, Risk: riskSvc, Hammer: hammerSvc, Config: cfg, Configs: configRepo, RiskControls: riskControlSvc})
	...
	return realtimeAuctionFixture{auctionID: auction.AuctionID, deposits: depositSvc, bids: bidSvc, hammers: hammerSvc, orders: orderSvc, risk: riskSvc, controls: riskControlSvc, depositRepo: depositRepo, orderRepo: orderRepo, auctionRepo: auctionRepo, realtime: realtime}
}
...
type recordingBidLiveAgentHook struct {
	ch chan recordingBidLiveAgentHookCall
}

type recordingBidLiveAgentHookCall struct {
	merchantID string
	sessionID  uint64
	bidderID   string
	bidderName string
...
func newRecordingBidLiveAgentHook() *recordingBidLiveAgentHook {
	return &recordingBidLiveAgentHook{ch: make(chan recordingBidLiveAgentHookCall, 4)}
}

func (r *recordingBidLiveAgentHook) EmitHighestBidWithBidderName(ctx context.Context, merchantID string, sessionID uint64, bidderID string, bidderName string, price int64) {
	_ = ctx
	r.ch <- recordingBidLiveAgentHookCall{merchantID: merchantID, sessionID: sessionID, bidderID: bidderID, bidderName: bidderName, price: price}
}

func (r *recordingBidLiveAgentHook) wait(t *testing.T) recordingBidLiveAgentHookCall {
	t.Helper()
	select {
	case got := <-r.ch:
		return got
	case <-time.After(time.Second):
		t.Fatal("timed out waiting for bid live agent hook")
	}
	return recordingBidLiveAgentHookCall{}
}

func (r *recordingBidLiveAgentHook) assertNoCall(t *testing.T, timeout time.Duration) {
	t.Helper()
	select {
	case got := <-r.ch:
		t.Fatalf("unexpected bid live agent hook: %+v", got)
