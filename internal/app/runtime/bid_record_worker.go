...
type BidRecordWriter struct {
	repo          auctionports.BidRepository
...
func NewBidRecordWriter(repo auctionports.BidRepository, log *redisinfra.EventLog, consumer string) *BidRecordWriter {
	if consumer == "" {
...
func WriteBidRecordIdempotent(ctx context.Context, repo auctionports.BidRepository, event redisinfra.BidEvent) (BidRecordWriteResult, error) {
	if event.EventType != "bid.accepted" || !event.Accepted {
...
func NewBidRecordReconciler(repo auctionports.BidRepository, log *redisinfra.EventLog) *BidRecordReconciler {
