...
| 代码依据 | 当前仓库源码，重点参考 `internal/app/server.go`、`internal/transport/http/ws_handler.go`、`internal/transport/ws/*.go`、`internal/modules/auction/app/{bid_service,service,hammer_service}.go`、`internal/app/runtime/timer.go`、`scripts/lua/{bid,hammer}.lua` |
...
| 端点 | `GET /ws/auctions/:auction_id`、`GET /ws/live-sessions/:session_id` |
...
| `GET /ws/live-sessions/:session_id` | 连接直播场次当前活跃拍品的实时事件流 | 先通过直播场次查询当前 `auctionId`，再订阅该拍品 Hub room |

注意：`/ws/auctions/:auction_id` 在建立连接时只校验路径参数与 JWT，不会先校验拍卖是否存在；拍卖不存在通常会在后续 `bid.place` 时通过 `bid.ack` 返回错误。`/ws/live-sessions/:session_id` 会在升级前校验直播场次和当前活跃拍品。
...
成功响应结构来自 `internal/modules/auth/app.LoginResult`：
...
| 出价服务、`bid.ack`、`bid.accepted`、`ranking.updated`、`timer.extended`、`bid.rejected` | `internal/modules/auction/app/bid_service.go` |
| 拍卖启动与 REST state | `internal/modules/auction/app/service.go`、`internal/transport/http/auction_handler.go` |
| 落槌与 `auction.closed` | `internal/modules/auction/app/hammer_service.go` |
| 定时器与 `timer.tick` | `internal/app/runtime/timer.go` |
| 风控事件 `risk.event` | `internal/modules/risk/app/service.go` |
| Lua 出价原子逻辑与拒绝原因 | `scripts/lua/bid.lua` |
| Lua 落槌原子逻辑 | `scripts/lua/hammer.lua` |
| 登录和 JWT Bearer 鉴权 | `internal/transport/http/auth_handler.go`、`internal/modules/auth/app/service.go` |
