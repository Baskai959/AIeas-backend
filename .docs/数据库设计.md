...
| `role` | VARCHAR(16) | NOT NULL DEFAULT 'buyer' | 角色：`buyer` / `merchant` / `admin` |
...
| `cap_price` | BIGINT | NOT NULL DEFAULT 0 | 封顶价（分），0 表示无封顶价；到达该价格自动成交 |
| `increment_rule` | JSON | NOT NULL | 加价规则：`type=fixed` 固定加价或 `type=ladder` 阶梯加价；`maxBidSteps` 表示单次最高加价步数 |
| `anti_sniping_sec` | INT | NOT NULL DEFAULT 15 | 反狙击触发窗口（秒） |
| `anti_extend_sec` | INT | NOT NULL DEFAULT 30 | 反狙击延长时长（秒） |
| `anti_extend_mode` | VARCHAR(16) | NOT NULL DEFAULT 'ADD' | 反狙击延时模式：`ADD`=结束时间增加 `anti_extend_sec`；`RESET`=倒计时重置为 `anti_extend_sec` |
...
| `start_time` | DATETIME(3) | NULL | 开拍时间；NULL 表示未设置定时开拍 |
| `end_time` | DATETIME(3) | NULL | 计划结束时间（可被反狙击延长）；NULL 表示启动时按时长计算 |
| `duration_sec` | INT | NOT NULL DEFAULT 0 | 拍卖时长（秒），0 表示未预设，可在上架/激活时指定 |
...
| `fulfillment_status` | VARCHAR(16) | NOT NULL DEFAULT 'UNSHIPPED' | `UNSHIPPED` / `SHIPPED` / `RECEIVED` |
...
| --- | --- | --- | --- |
| `user` | `PRIMARY(id)` | 主键 | |
| `user` | `uk_account(account)` | 唯一 | 账号唯一登录 |
| `user` | `uk_phone(phone)` | 唯一 | 手机号唯一 |
...
HAMMER_PENDING -- 落锤中（短暂中间态，受 lock:close 保护）
  ↓ 有最高价 + ≥ reserve_price
CLOSED_WON     -- 成交
  ↑ 出价达到 cap_price 时由 Lua 直接置为 CLOSED_WON，并由后端补齐订单/押金落库
  ↓ 无人出价 / 流拍 / 未达 reserve
CLOSED_FAILED  -- 流拍
  ↓ 订单 + 保证金 + 资金清算完成
SETTLED        -- 结算完毕（终态）
