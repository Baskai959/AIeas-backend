...
| `durationSec` | integer | 条件必填 | `startExplain` 时必须提供，表示拍卖/讲解时长，单位秒，必须大于 0 |
| `startTime` | string | 否 | `startExplain` 时可传，RFC3339 时间；未来时间表示预约开拍，不传表示立即开拍 |
...
| `startExplain` | 开始拍卖/讲解 | 激活直播场次中的拍品，成为当前讲解/拍卖拍品；必须传 `durationSec`，可传 `startTime` 预约开拍 |
