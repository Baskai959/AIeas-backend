...
> 说明：本文档是当前只读 MCP 的设计和接入说明。直播控制能力已拆到独立 MCP，商家直播控制类工具请以 [MCP-live-control.md](./MCP-live-control.md) 为准。
...
- 读取拍品实时状态、直播场次统计、风险/审计只读数据。
- 不开放创建、更新、删除、出价、落槌、支付、审核、封禁等写操作。

协议基线：

- MCP 数据层使用 JSON-RPC 2.0。
- 远程服务使用 Streamable HTTP 风格入口，只读 endpoint 为 `/mcp/read`；当前只读实现使用 `POST /mcp/read` 处理 JSON-RPC 调用，`GET /mcp/read` 暂不启用服务端主动流，返回 405。
...
MCP 是新的协议适配层，不是新的业务实现层。它只能调用模块 application/read model 边界，不直接访问 MySQL、Redis、GORM 或 Lua 脚本。
...
MCP Client / Host
  ...
  -> internal/modules/*/app
       - existing module application services
       - optional MCPReadService facade
  -> internal/repository
...
所有 MCP 协议接口放在 `internal/transport/mcp` 包内，集中注册，避免散落到 HTTP handler 或模块 app 文件中。

```text
internal/
  app/
    server.go
      # 唯一装配点：构造 MCP read facade、handler，挂载 /mcp/read

  modules/
    mcp/
      app/
        service.go
          # 只读/控制 facade，不包含 MCP/JSON-RPC transport 细节
          # 负责跨模块组合：直播场次详情、成交汇总、商家概览等

  transport/
    mcp/
      handler.go
        # Hertz endpoint：POST /mcp/read；GET /mcp/read 显式返回 405
      registry.go
...
- `/mcp/read` 与 `/api/v1` 同级挂载，不放在 `/api/v1` 下。
- 继续复用全局 `RecoveryMiddleware`、`RequestIDMiddleware`、`RateLimiter`、`AuditMiddleware`。
- MCP 自己校验 `X-API-Key`，不复用用户 `accessToken`。鉴权成功后使用配置中的 `mcp.read.actorID` / `mcp.read.actorRole` 作为模块 application 层调用身份。
...
`GET /mcp/read` 已挂载，但第一阶段不提供服务端主动消息流；客户端应使用 `POST /mcp/read` 完成 `initialize`、`resources/read`、`tools/call` 等只读调用。

Headers：

| Header | 必填 | 说明 |
|---|---|---|
| `X-API-Key: <apiKey>` | 是 | 只读 MCP 专用 API Key，来自 `mcp.read.apiKey` 或 `MCP_READ_API_KEY`。 |
...
可通过环境变量覆盖：`MCP_READ_API_KEY`、`MCP_READ_ACTOR_ID`、`MCP_READ_ACTOR_ROLE`。`actorRole` 可取 `buyer`、`merchant`、`admin`；生产环境建议使用 `admin` 或专门的只读服务账号，并把 API Key 放在密钥管理系统或运行时环境变量中。
...
| 直播场次 | 仅可读 `LIVE` 场次详情；列表禁止 | 本商家场次 | 全部 |
| 场次出价记录 | 禁止 | 本商家场次 | 全部 |
...
- MCP 不接受请求参数里的 `actorId`、`actorRole`，调用身份只来自后端配置的 `mcp.read.actorID` / `mcp.read.actorRole`。
...
```json
{
  "schemaVersion": "aieas.mcp.read.v1",
  "traceId": "trc_...",
  "data": {}
}
```
  ...
  "contents": [
    {
      ...
      "text": "{\"schemaVersion\":\"aieas.mcp.read.v1\",\"traceId\":\"trc_...\",\"data\":{}}"
    }
  ]
}
```
...
| `aieas://auction-lots?sellerId={sellerId}&status={status}&liveSessionId={liveSessionId}&limit={limit}&offset={offset}` | `auction-lots-list` | 拍品列表。 | merchant/admin；buyer 仅公开直播场次范围 |
...
| `read_auction_lots` | 查询拍品列表。 | `sellerId,status,liveSessionId,limit,offset` |
| `read_auction_lot` | 读取拍品详情。 | `auctionId` |
| `read_auction_state` | 读取拍品实时状态。 | `auctionId` |
| `read_live_sessions` | 查询直播场次。 | `merchantId,status,limit,offset` |
...
1. 新增 `internal/transport/mcp` 包骨架和 `/mcp/read` endpoint。
