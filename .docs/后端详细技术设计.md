...
> 技术基线：Go 1.22+ + CloudWeGo Hertz + Hertz WebSocket（`github.com/hertz-contrib/websocket`）+ GORM（或 sqlc）+ go-redis/v9 + MySQL 8 + Redis 7。本文仅描述 Go 后端服务设计，所有金额使用 `BIGINT`，单位为分，起拍价允许为 0。
...
| 层级 | 推荐选型 | 备选 | 不推荐 / 不主推 | 选型理由 |
| --- | --- | --- | --- | --- |
| Go 版本 | Go 1.22+ | Go 1.21+ | 低于 Go 1.20 | Go 1.22+ 标准库、性能和工具链稳定，适合高并发网络服务。 |
| HTTP 框架 | CloudWeGo Hertz | Gin / Echo / Fiber / chi | go-zero / 仅原生 `net/http` 方案 | 采用 Hertz 作为主 HTTP 框架，性能、路由分组、中间件和 CloudWeGo 生态更适合高并发竞拍服务；Gin/Echo/Fiber/chi 仅作为迁移备选。 |
| WebSocket | Hertz WebSocket：`github.com/hertz-contrib/websocket` | `github.com/gorilla/websocket` / `nhooyr.io/websocket` | 封装型 RPC 网关 | 采用 Hertz WebSocket 与 HTTP 服务统一承载，便于复用 Hertz 中间件、鉴权上下文、连接升级、心跳、背压、房间广播和断线重连语义。 |
...
```mermaid
flowchart LR
    U[用户端 / 商家端 / 管理端] -->|REST| G[Hertz HTTP Server]
...
flowchart TB
    ...
    Repo --> Service[Module App Services]
    Script --> Service
    Redis --> Service
    DB --> Repo
    Service --> Hub[WS Hub]
    Service --> Cron[Scheduler]
    Hub --> Router[Hertz Router]
    Service --> Router
...
sequenceDiagram
    ...
    participant API as Hertz /api/v1/auth/login
    participant Auth as AuthService (modules/auth/app)
...
sequenceDiagram
    ...
    participant API as Hertz /api/v1/items
...
sequenceDiagram
    ...
    participant AuctionSvc as AuctionService (modules/auction/app)
...
sequenceDiagram
    ...
    participant BidSvc as BidService (modules/auction/app)
...
sequenceDiagram
    ...
    participant HammerSvc as HammerService (modules/auction/app)
...
func (r *Room) broadcast(msg Envelope) {
    ...
    for _, c := range r.clients {
        ...
        case c.send <- msg:
        default:
            go c.CloseWithReason(websocket.ClosePolicyViolation, "slow consumer")
        }
    }
}
```

### 6.8 Hertz WebSocket 接入示例

WebSocket 升级入口由 Hertz 路由承载，复用 JWT、RequestID、审计等 Hertz middleware，业务消息仍进入自建 Hub：
...
func RegisterWebSocket(h *server.Hertz, hub *ws.Hub) {
    upgrader := websocket.HertzUpgrader{
        ReadBufferSize:  4096,
        WriteBufferSize: 4096,
        CheckOrigin: func(ctx context.Context, c *hertzapp.RequestContext) bool {
            return true
        },
    }

    h.GET("/ws/auctions/:auction_id", func(ctx context.Context, c *hertzapp.RequestContext) {
        user, err := middleware.AuthenticateWS(ctx, c)
        if err != nil {
        ...
        _ = upgrader.Upgrade(c, func(conn *websocket.Conn) {
            hub.Attach(ctx, ws.AttachCommand{Conn: conn, User: user, AuctionID: auctionID})
        })
    })
}
```
...
func main() {
    ...
    app.RegisterRoutes(h, cfg, logger, services, hub)
...
### 12.3 Hertz 路由、中间件与 Handler 签名示例

`internal/app/router.go` 使用 Hertz 路由组注册 REST 与 WebSocket 入口，Handler 签名统一为 `func(ctx context.Context, c *app.RequestContext)`：
...
func RegisterRoutes(h *server.Hertz, cfg config.Config, logger *zap.Logger, services *service.Services, hub *ws.Hub) {
    h.Use(middleware.RequestID(), middleware.Recovery(logger))

    v1 := h.Group("/api/v1")
    v1.POST("/auth/login", func(ctx context.Context, c *hertzapp.RequestContext) {
        req := new(dto.LoginRequest)
        if err := c.BindAndValidate(req); err != nil {
            c.JSON(consts.StatusBadRequest, response.Fail(domain.ErrParamInvalid))
            return
        }
        result, err := services.Auth.Login(ctx, req)
        if err != nil {
            c.JSON(consts.StatusUnauthorized, response.Fail(err))
            return
        }
        c.JSON(consts.StatusOK, response.Success(result))
    })

    auth := v1.Group("/", middleware.JWTAuth(services.Auth))
    auth.POST("/auctions/:id/hammer", handlers.NewAuctionHandler(services).Hammer)
    auth.POST("/auctions/:id/enroll", handlers.NewAuctionHandler(services).Enroll)
    v1.GET("/auctions/:id/state", handlers.NewAuctionHandler(services).State)

    RegisterWebSocket(h, hub)
}
```
...
6. 启动 Hertz HTTP server、Hertz WebSocket Hub、cron 调度器。
